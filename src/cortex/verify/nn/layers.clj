...
(def activation-answers
  {:logistic [[0.2689414213699951 0.7310585786300049 0.2689414213699951 0.7310585786300049
   ...
   :selu [ [-1.1113306283950806 1.0507010221481323 -1.1113306283950806 1.0507010221481323
            -1.1113306283950806 1.0507010221481323 -1.1113306283950806 1.0507010221481323
            -1.1113306283950806 1.0507010221481323]
...
(defn test-activation
  ...
  (let [item-count 10
        ...
        (forward-backward-test [(layers/input item-count)
                               ...
                               (flatten (repeat (/ item-count 2) [-1 1]))
                               (flatten (repeat (/ item-count 2) [-1 1])))]
    (println "Carin first " (mapv str output))
    (println "Carin second " (mapv str input-gradient))
